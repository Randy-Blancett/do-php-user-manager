Ext.define('darkowl.desktop.cStartMenu', {
	...
	addMenuItem : function(str_Text, obj_Menu, obj_Data) {
		...
		if (!obj_Current) {
			...
			if (obj_Data.iconCls) {
				obj_NewItem.iconCls = obj_Data.iconCls;
			}
	...
	addStartMenu : function(obj_Data) {
		...
		if (obj_Data.special) {
			for (key in this.m_arr_MenuItems) {
				this.m_obj_Toolbar.remove(this.m_arr_MenuItems[key]);
			}
			...
			for (key in this.m_arr_MenuItems) {
				this.m_obj_Toolbar.add(this.m_arr_MenuItems[key]);
			}
		}
	},
