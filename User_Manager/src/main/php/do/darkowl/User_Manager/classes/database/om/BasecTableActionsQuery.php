...
abstract class BasecTableActionsQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'user_manager1', $modelName = 'cTableActions', $modelAlias = null)
	{
	 ...
	 *
	 * @return    cTableActions|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			cTableActionsPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
