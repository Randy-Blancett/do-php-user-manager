 ...
 *
 * @method     cTableApplications findOneById(string $id) Return the first cTableApplications filtered by the id column
...
abstract class BasecTableApplicationsQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'user_manager1', $modelName = 'cTableApplications', $modelAlias = null)
	{
	 ...
	 *
	 * @return    cTableApplications|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			cTableApplicationsPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
