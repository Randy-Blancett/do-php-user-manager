...
abstract class BasecTableGroupsQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'user_manager1', $modelName = 'cTableGroups', $modelAlias = null)
	{
	 ...
	 *
	 * @return    cTableGroups|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			cTableGroupsPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
