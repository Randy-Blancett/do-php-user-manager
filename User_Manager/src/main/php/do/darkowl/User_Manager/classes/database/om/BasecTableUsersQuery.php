 ...
 *
 * @method     cTableUsers findOneById(string $id) Return the first cTableUsers filtered by the id column
...
abstract class BasecTableUsersQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'user_manager1', $modelName = 'cTableUsers', $modelAlias = null)
	{
	 ...
	 *
	 * @return    cTableUsers|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new cTableUsers();
			$obj->hydrate($row);
			cTableUsersPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
