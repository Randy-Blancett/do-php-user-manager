 ...
 *
 * @method     cTableKeybox findOneById(string $id) Return the first cTableKeybox filtered by the id column
...
abstract class BasecTableKeyboxQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'user_manager1', $modelName = 'cTableKeybox', $modelAlias = null)
	{
	 ...
	 *
	 * @return    cTableKeybox|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			cTableKeyboxPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
